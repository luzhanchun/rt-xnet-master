...
def paired_paths_from_lmdb(folders, keys):
    ...
    assert len(folders) == 3, (
        'The len of folders should be 3 with [input_folder, gt_folder, th_folder]. '
        f'But got {len(folders)}')
    assert len(keys) == 3, (
        'The len of keys should be 3 with [input_key, gt_key, th_key]. '
        f'But got {len(keys)}')

    input_folder, gt_folder, th_folder = folders
    input_key, gt_key, th_key = keys

    if not (input_folder.endswith('.lmdb') and gt_folder.endswith('.lmdb') and th_folder.endswith('.lmdb')):
        raise ValueError(
            f'{input_key}, {gt_key}, and {th_key} folders should all be in lmdb format. '
            f'Received: {input_key}: {input_folder}; '
            f'{gt_key}: {gt_folder}; '
            f'{th_key}: {th_folder}')

    # Ensure all meta_info files are aligned
    def read_lmdb_keys(folder):
        with open(osp.join(folder, 'meta_info.txt')) as fin:
            return [line.split('.')[0] for line in fin]

    input_lmdb_keys = read_lmdb_keys(input_folder)
    gt_lmdb_keys = read_lmdb_keys(gt_folder)
    th_lmdb_keys = read_lmdb_keys(th_folder)

    if set(input_lmdb_keys) != set(gt_lmdb_keys) or set(input_lmdb_keys) != set(th_lmdb_keys):
        raise ValueError(
            f'Keys in {input_key}, {gt_key}, and {th_key} folders are not identical.')
...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    input_paths = sorted(scandir(input_folder))
    gt_paths = sorted(scandir(gt_folder))
    th_paths = sorted(scandir(th_folder))

    assert len(input_paths) == len(gt_paths) == len(th_paths), (
        f'{input_key}, {gt_key}, and {th_key} datasets have different number of images: '
        f'{len(input_paths)}, {len(gt_paths)}, {len(th_paths)}.')

    paths = []
    for idx in range(len(gt_paths)):
        # GT
        gt_file = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_file))
        gt_full_path = osp.join(gt_folder, gt_file)

        # Input
        input_file = input_paths[idx]
        basename_input, ext_input = osp.splitext(osp.basename(input_file))
        input_name = f'{filename_tmpl.format(basename)}{ext_input}'
        input_full_path = osp.join(input_folder, input_name)

        # Thermal
        th_file = th_paths[idx]
        basename_th, ext_th = osp.splitext(osp.basename(th_file))
        th_name = f'{filename_tmpl.format(basename)}{ext_th}'
        th_full_path = osp.join(th_folder, th_name)

        assert osp.exists(input_full_path), f'{input_name} is missing in {input_folder}'
        assert osp.exists(gt_full_path), f'{basename}{ext} is missing in {gt_folder}'
        assert osp.exists(th_full_path), f'{th_name} is missing in {th_folder}'

        paths.append(
            dict([
                (f'{input_key}_path', input_full_path),
                (f'{gt_key}_path', gt_full_path),
                (f'{th_key}_path', th_full_path),
            ])
        )
    return paths
