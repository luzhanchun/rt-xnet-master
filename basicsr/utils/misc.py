...
def mkdir_and_rename2(path, rename_flag):
    """mkdirs. If path exists, rename it with timestamp and create a new one.
...
def make_exp_dirs(opt):
    ...
    try:
        l = len(os.listdir(
            osp.join(opt['path']['experiments_root'], 'models')))
        opt['rename_flag'] = l != 0
    except:
        logger.info("rename_flag {}".format(opt['rename_flag']))

    if opt['is_train']:
        mkdir_and_rename2(path_opt.pop('experiments_root'), opt["rename_flag"])
    else:
...
def scandir(dir_path, suffix=None, recursive=False, full_path=False):
    ...
    dir_path = os.path.join("basicsr",dir_path)
    root = dir_path
